...
constexpr float CURVATURE_ANGLE_THRESH = 0.10f;
...
// extract arc points from curved contour 
void extractCurvedArcPoints(
    ...
    for (int i = CURVATURE_K; i < n - CURVATURE_K; ++i) {
        ...
        constexpr float COS_THRESH = std::cos(CURVATURE_ANGLE_THRESH);
...
int main(int argc, char** argv) {
    ...
    // mode 3: debug forced geometric, compare algebraic and geometric performance.
        ...
        std::vector<cv::Point2f> arcPts;
        extractCurvedArcPoints(contours[idx], arcPts);
        usedAlgebraic = false;
    ...
    if (mode == 3) {
        ...
        for (const auto& p : arcPts)
            cv::circle(out, p, 2, cv::Scalar(0,0,255), -1);
    }
