...
static bool ransacCircleFit(
    ...
    return bestInliers >= params.ransacMinInliers && bestRMS <= params.ransacMaxRms;
}
...
bool detectCircle(
    ...
    // FIRST TRY ALGEBRAIC CIRCLE FIT
    if (detail::algebraicCircleFit(arcPts, params, result.center, result.radius)) {
        result.rms = detail::computeRmsError(arcPts, result.center, result.radius);
    ...
    // FALLBACK TO RANSAC CIRCLE FIT
    if (!fitSucceeded ) {
        fitSucceeded = detail::ransacCircleFit(arcPts, params, result.center, result.radius, result.rms);
    }
