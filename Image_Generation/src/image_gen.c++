...
int main() {
    ...
    std::cout << "Base sun image path (e.g., base_images/sun.png): ";
    std::string basePath;
    std::cin >> basePath;
    
    int sunW, sunH, sunC;
    unsigned char* sunImg =
        stbi_load(basePath.c_str(), &sunW, &sunH, &sunC, 4);

    if (!sunImg) {
        std::cerr << "Failed to load " << basePath << "\n";
        return 1;
    }
    ...
    while (true) {
        ...
        oss << "gen_images/gen_sun_" << index << ".png";
        filename = oss.str();
